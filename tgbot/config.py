...
class TgBot:
    ...
    def from_env(env: Env):
        ...
        chat_errors = env.str("CHAT_ERRORS")
        channels_check = env.str("CHANNELS_CHECK")
        return TgBot(
            token=token, admin_ids=admin_ids, use_redis=use_redis,
            chat_errors=chat_errors, channels_check=channels_check)
...
class InfluxDb:
    ...
    def from_env(env: Env):
        """
        Creates the InfluxDb object from environment variables.
        """
        ...
        return InfluxDb(
            token=token, url=url, org=org, bucket=bucket)
...
class Config:
    ...
    influx_db: Optional[InfluxDb] = None
...
def load_config(path: str = None) -> Config:
    ...
    return Config(
        ...
        misc=Miscellaneous(),
        influx_db=InfluxDb.from_env(env),
    )
